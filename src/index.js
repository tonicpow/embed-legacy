...
TonicPow.iframeLoader = async () => {
  ...
  for (let i = tonicDivs.length - 1; i >= 0; i--) {
    ...
    if (typeof dataAddress === 'undefined' || !dataAddress || dataAddress.length < 27) {
      if (stickerAddress) {
    ...
    iframe.src = networkUrl + '/' + loadState + '?' +
      ...
      (stickerAddress && !dataAddress ? '&sticker_address=' + stickerAddress : '') +
      (stickerTx && stickerAddress ? '&sticker_tx=' + stickerTx : '') +
      '&rate=' + rate +
      '&currency=' + currency +
      '&width=' + displayWidth +
